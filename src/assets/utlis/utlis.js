...
export const renderHeader = (filters, setFilters, setGlobalFilterValue, globalFilterValue, selectedCity, setSelectedCity, cities) => {
    return (
        <div className="flex justify-center items-center">

            <Link to={'products-add'} style={{ width: 'fit-content', marginRight: "20px" }} >
                <div className='bg-[var(--min-color)] text-white p-4 py-2 w-fit rounded-[6px] flex justify-between items-center'>
                    ...
                    <span className='ms-2 text-[12px]'>Add Battery</span>
                </div>
            </Link>

            <div className='flex items-center mx-2 flex-1' style={{ width: "calc(100% / 4)" }}>
                <h3 className='text-[#CFCFCF] text-[12px]'>Search:</h3>
                <InputText value={globalFilterValue} className='!p-0 !w-full !text-[#8b6464] !bg-transparent !rounded-[6px] !border-2 !border-[#8b6464] !px-2 h-[38px] !ms-2' onChange={(e) => onGlobalFilterChange(e, filters, setFilters, setGlobalFilterValue)} placeholder="Product Name" />
            </div>

            <div className='flex items-center mx-2 flex-1' style={{ width: "calc(100% / 4)" }}>
                <h3 className='text-[#8b6464] text-[12px]'>Marka:</h3>
                <Dropdown value={selectedCity} style={{ width: 'calc(100% - 70px)' }} onChange={(e) => setSelectedCity(e.value)} options={cities} optionLabel="name"
                    placeholder="Hyundai" />
            </div>
            <div className='flex items-center mx-2 flex-1' style={{ width: "calc(100% / 4)" }}>
                <h3 className='text-[#8b6464] text-[12px] min-w-[100px]'>BATT-COMPANY</h3>
                <Dropdown value={selectedCity} style={{ width: 'calc(100% - 70px)' }} onChange={(e) => setSelectedCity(e.value)} options={cities} optionLabel="name"
                    placeholder="Moura" />
            </div>
        </div>
